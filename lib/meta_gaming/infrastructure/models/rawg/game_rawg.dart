...
class GameRawg {
  ...
  factory GameRawg.fromJson(Map<String, dynamic> json) => GameRawg(
        ...
        ratings: List<RatingRawg>.from(
            json["ratings"].map((x) => RatingRawg.fromJson(x))),
        ratingsCount: json["ratings_count"],
        reviewsTextCount: json["reviews_text_count"],
        added: json["added"],
        addedByStatus:
            AddedByStatusRawg.fromJson(json["added_by_status"] ?? {}),
        ...
        platforms: List<PlatformElementRawg>.from((json["platforms"] ?? [])
            .map((x) => PlatformElementRawg.fromJson(x))),
        parentPlatforms: List<ParentPlatformRawg>.from(
            (json["parent_platforms"] ?? [])
                .map((x) => ParentPlatformRawg.fromJson(x))),
        genres: List<GenreRawg>.from(
            json["genres"].map((x) => GenreRawg.fromJson(x))),
        stores: List<StoreRawg>.from(
            json["stores"]?.map((x) => StoreRawg.fromJson(x)) ?? []),
        clip: json["clip"],
        tags: List<GenreRawg>.from(
            json["tags"].map((x) => GenreRawg.fromJson(x))),
        esrbRating: EsrbRatingRawg.fromJson(
            json["esrb_rating"] ?? {'rating': 'Not Rated'}),
        shortScreenshots: List<ShortScreenshotRawg>.from(
            json["short_screenshots"]
                .map((x) => ShortScreenshotRawg.fromJson(x))),
      );
...
final colorValues = EnumValuesRawg({"0f0f0f": Color.THE_0_F0_F0_F});
